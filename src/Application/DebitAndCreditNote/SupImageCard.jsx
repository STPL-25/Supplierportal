...
//   }, []);
  ...
  const fetchInvDetails = async () => {
    if (!userRole) return; // Ensure selectedSupplier is valid
    try {
      const payload = {
...
// console.log(companyName==="test private limited")
  const fetchImagesData = async () => {
    ...
    try {
      ...
      const { KTMCredit, KTMDebit, supDebitAcceptImg, supDebitRejectImg, supCreditAcceptImg, supCreditRejectImg, id } = response.data;
      ...
      setImgCards((prevalue) => ({
        ...
        debitNote: KTMCredit,
        creditNote: KTMDebit,
        accDebitNote: supDebitAcceptImg,
        rejDebitNote: supDebitRejectImg,
        accCreditNote: supCreditAcceptImg,
        rejCreditNote: supCreditRejectImg
      }));
    } catch (error) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.put(`${API}/debitandCredit/actiondebitandcreditdetails/supplier`, { selectedInvoice, accImgs, rejImgs })
            ...
            <div key={noteType} className="space-y-4">
              ...
              {imgCards[noteType]?.length > 0 && selectedStatus === "Pending" && (
                      ...
                      .map((img, index) => (
                        ...
                        >
                          ...
                          <div className="flex justify-center space-x-4 p-4 bg-gray-50">
                            ...
                            <button 
                              type="button"
                              onClick={() => handleAccept(img, noteType,noteType==="creditNote"?"supCredit":"supDebit")}
                            ...
                            <button 
                              type="button"
                              onClick={() => handleReject(img, noteType,noteType==="creditNote"?"supCredit":"supDebit")}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {console.log("8888888888888888888888888",imgCards[`acc${noteType.charAt(0).toUpperCase() + noteType.slice(1)}`])}
