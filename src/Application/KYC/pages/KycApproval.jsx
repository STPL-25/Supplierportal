...
// import eyeIcon from "../../Diamond/assets/eyeicon.png";
...
//     const [rejectReason, setRejectReason] = useState(""); // Capture reject reason
//     const [openRejectDialog, setOpenRejectDialog] = useState(false); // Toggle dialog visibility
//     const [isVerified, setIsVerified] = useState(false); // To track if Verified is clicked
//     const [isRejected, setIsRejected] = useState(false); // T
...
//             // Set error message
//             // setErrorMsg("Failed to fetch KYC data.");
...
//     useEffect(() => {
//         if (supplierDetails) {
//             // Only fetch data if supplierDetails exists
//             fetchKycData();
//         } else {
//             // Clear accordion state when no supplier is selected
//             setOpenAccordions([]);
...
//     const { data: supplierNames, error: supplierError, isLoading: supplierLoading ,refetch: refetchSupplierNames} = useQuery({
...
//         },
//         onSuccess: () => {
//             // Optional: Uncomment for success feedback
//             // setSuccessMsg("Supplier names loaded successfully.");
...
//             setIsVerified(true);
//             setIsRejected(false);
//             const response = await axios.put(`${KYC_API}/kycdatachecking/${supplierId}/${action}`);
//             if (response.status === 200) { // Use response.status instead of response.data
...
//                     //  socket.emit('approval', { supplierId, action });
...
//                 // Refetch supplier names to update the dropdown
...
//             }
//             console.log(response.data);
//             // socket.emit('approval', { supplierId, action,user });
...
//             const response = await axios.put(`${KYC_API}/kycdatachecking/${supplierId}/Reject`, { reason: rejectReason, RejectectedBy: encodedUserRole });
...
//                         {HeaderData.map((section, sectionIndex) => (
...
//                         {userRole === "Diamond-Purchase" || userRole === "Gold-Purchase"||userRole === "Silver-Purchase" ? (
//                             <>
//                                 <Checkbox
//                                     checked={isChecked}
//                                     onChange={handleCheckboxChange}
...
//                 </div>
//             )}
//             <Dialog open={openRejectDialog && isRejected} onClose={handleCloseRejectDialog}>
